...
@section('content')
    <div class="flex justify-between items-center mb-4">
        ...
        <a href="{{ route('admin.footer-links.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded">Add Link</a>
    </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($links as $link)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $link->section->title ?? '-' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $link->title }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-blue-600"><a href="{{ $link->url }}" target="_blank" class="hover:underline">{{ $link->url }}</a></td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-4 text-center text-sm text-gray-500">No footer links found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
