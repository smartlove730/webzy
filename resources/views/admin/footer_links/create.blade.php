...
@section('content')
    <div class="bg-white p-6 rounded-lg shadow-md">
        <form action="{{ route('admin.footer-links.store') }}" method="POST">
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="footer_section_id" id="footer_section_id" required
                        ...
                        @foreach($sections as $section)
                            <option value="{{ $section->id }}" {{ (old('footer_section_id', $selectedSectionId ?? null) == $section->id) ? 'selected' : '' }}>
                                {{ $section->title }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="mt-6">
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Save Link</button>
